module Rubyplot
  module Magick
    class Scatter < Rubyplot::Artist
      ...
      # x_dta_points.length != y_data_points.length::
      ...
      #
      def data(x_data = [], y_data = [], label: :default, color: :default)
        # Call the existing data routine for the y axis data
        super(y_data, label: label, color: color)

        # append the x data to the last entry that was just added in the @data member
        last_elem = @data.length - 1
        @data[last_elem] << x_data

        if @geometry.maximum_x_value.nil? && @geometry.minimum_x_value.nil?
          @geometry.maximum_x_value = @geometry.minimum_x_value = x_data.first
        end

        @geometry.maximum_x_value = x_data.max > @geometry.maximum_x_value ?
                                      x_data.max : @geometry.maximum_x_value
        @geometry.minimum_x_value = x_data.min < @geometry.minimum_x_value ?
                                      x_data.min : @geometry.minimum_x_value
      end
...
=======
    ...
    def draw
      super
      return unless @geometry.has_data
>>>>>>> 3a3e050d229d2fa5d421e8562ab1685ee0ee01ce
